...
description: "Runs the Lighthouse Next-Best-Action Recommendation analysis for Predictive Modelling. Use when the analyst is asked to run `next-best-action` or investigate predict the next optimal user touchpoint (push, promo, genre recommendation) to maximize long-term retention."
---
...
- Skill key: `next-best-action`
- Group: `predictive-modelling` (Predictive Modelling)
- Owning agent: `predictive-analyst`
- Primary metric hint: `next_best_action_index`

## When To Use

Use this skill when the user selects `next-best-action` or asks for: Predict the next optimal user touchpoint (push, promo, genre recommendation) to maximize long-term retention.
