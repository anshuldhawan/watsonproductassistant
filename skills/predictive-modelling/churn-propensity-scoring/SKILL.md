---
name: churn-propensity-scoring
description: "Runs the Lighthouse User Churn Propensity Scoring analysis for Predictive Modelling. Use when the analyst is asked to run `churn-propensity-scoring` or investigate calculate probability of a user churning in the next 7 days based on recent usage decay and skip rates."
---

# Skill: User Churn Propensity Scoring

## Identity

- Skill key: `churn-propensity-scoring`
- Group: `predictive-modelling` (Predictive Modelling)
- Owning agent: `predictive-analyst`
- Primary metric hint: `churn_probability`

## When To Use

Use this skill when the user selects `churn-propensity-scoring` or asks for: Calculate probability of a user churning in the next 7 days based on recent usage decay and skip rates.
