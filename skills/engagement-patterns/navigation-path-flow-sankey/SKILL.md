---
name: navigation-path-flow-sankey
description: "Runs the Lighthouse Navigation Path Flow (Sankey) analysis for Engagement & Usage Patterns. Use when the analyst is asked to run `navigation-path-flow-sankey` or investigate visual flow mapping of user clicks leading up to a subscription checkout or play click."
---

# Skill: Navigation Path Flow (Sankey)

## Identity

- Skill key: `navigation-path-flow-sankey`
- Group: `engagement-patterns` (Engagement & Usage Patterns)
- Owning agent: `engagement-analyst`
- Primary metric hint: `navigation_path_flow_sankey_index`

## When To Use

Use this skill when the user selects `navigation-path-flow-sankey` or asks for: Visual flow mapping of user clicks leading up to a subscription checkout or play click.
