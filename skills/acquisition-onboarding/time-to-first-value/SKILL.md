...
description: "Runs the Lighthouse Time-to-First-Value (TTFV) analysis for Acquisition & Onboarding. Use when the analyst is asked to run `time-to-first-value` or investigate calculate duration and user actions required before a user experiences the core product value (first full song play)."
---

# Skill: Time-to-First-Value (TTFV)

## Identity

- Skill key: `time-to-first-value`
- Group: `acquisition-onboarding` (Acquisition & Onboarding)
- Owning agent: `acquisition-analyst`
- Primary metric hint: `time_to_first_value_index`

## When To Use

Use this skill when the user selects `time-to-first-value` or asks for: Calculate duration and user actions required before a user experiences the core product value (first full song play).
